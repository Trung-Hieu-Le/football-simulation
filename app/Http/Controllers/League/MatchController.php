...
class MatchController extends Controller
{
    ...
    public function index($seasonId)
    {
        ...
        $matches = Match::where('season_id', $seasonId)
                       ->with(['team1', 'team2'])
                       ->orderBy('division')
                       ->orderBy('round')
                       ->get()
    ...
    protected function simulateMatch(Match $match, string $seasonMeta): void
    {
    ...
    {
        $match = Match::with(['team1', 'team2', 'season'])->findOrFail($matchId);
    ...
    {
        Match::where('season_id', $seasonId)
             ->where('round', $round)
             ->update([
                 'team1_score' => null,
                 'team2_score' => null,
